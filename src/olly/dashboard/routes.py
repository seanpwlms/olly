...
from olly.state import StateStore, open_state
...
@contextmanager
def _state_db() -> Generator[tuple[StateStore, str], None, None]:
    config = load_config()
    # Use the first connection for dashboard state
    ...
    with open_state(config, adapter, first_nc.connection.type) as state_db:
...
def index(request: Request):
    ...
    with _state_db() as (state_db, conn_name):
...
def _build_table_rows(state_db: StateStore, connection_name: str = "") -> list[dict]:
...
def tables(
    ...
    with _state_db() as (state_db, conn_name):
        table_rows = _build_table_rows(state_db, conn_name)
